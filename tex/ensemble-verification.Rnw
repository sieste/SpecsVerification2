...
set.seed(31415)
options(prompt=" ", continue=" ", digits=4, scipen=10)
@
...
plot(NA, type='n', xlim=range(yrs), ylim=range(c(obs, ens)), 
     xlab="", ylab="temp [C]")
for (i in 1:ncol(ens)) 
  points(yrs, ens[, i], col="darkgray", cex=.7)
...
\begin{center}
%
<<label=rank-hist, fig=TRUE, width=5, height=2.5, keep.source=FALSE>>=
...
\begin{center}
%
<<label=rank-hist-pp, fig=TRUE, width=5, height=3.5, keep.source=FALSE>>=
...
\begin{center}
%
<<label=reldiag, fig=TRUE, width=4, height=4.5, keep.source=FALSE>>=
