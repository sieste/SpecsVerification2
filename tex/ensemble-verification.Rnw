...
\Shorttitle{\proglang{R}-package \pkg{SpecsVerification}} %% a short title (if necessary)
\Plainauthor{Stefan Siegert} %% comma-separated
\Plaintitle{Forecast Verification: Ensemble-adjusted scoring rules, graphical tools, comparative verification, and uncertainty quantification implemented in the R package SpecsVerification}
...
This paper summarises methods to assess sampling uncertainty for a large number of forecast verification metrics and scores, including scoring rules, the correlation coefficient, the area under the ROC curve, the rank histogram, the reliability diagram, and the Reliability-Resolution-Uncertainty decomposition of the Brier score.
Forecast verification is often more meaningful by comparing competing forecasting systems for the same observation.
A number of methods for uncertainty assessment for comparative verification are summarised.
...
The package documented in this paper is an updated, yet unpublished version of \pkg{SpecsVerification}, with new and rewritten functions that are not available in the current package version.
The source code of the revised version of \pkg{SpecsVerification} documented in this paper is publicly available in the github repository \url{https://github.com/sieste/SpecsVerification2}.
...
The main contribution of this paper is to summarise ensemble verification scores, and document their implementation in the \proglang{R} statistical programming environment \citep{R2015}.
...
\pkg{SpecsVerification} contains a number of functions to enable comparative verification. 
...
All verification functions implemented in \pkg{SpecsVerification} provide uncertainty estimates by p-values, confidence intervals, and resampling methods.
...
While \pkg{verification} contains a limited number of functions for ensemble verification, comparative verification and uncertainty quantification, it does not have the same focus on these topics as does \pkg{SpecsVerification}.
...
The focus of \pkg{SpecsVerification} is on comparative verification of ensemble forecasts, and uncertainty quantification by variance estimation, statistical testing, and confidence intervals.
