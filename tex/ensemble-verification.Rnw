...
\begin{figure}
\begin{center}
<<ens-brier, fig.width=4, fig.height=3, echo=FALSE>>=
brier <- sapply(2:60, function(RR) c(
  mean(EnsBrier(ens.bin[, i.small], obs.bin, R.new=RR)),
  mean(EnsBrier(ens.bin[, i.large], obs.bin, R.new=RR))))
matplot(cbind(2:60, 2:60), t(brier), type="l", col="black", lty=c(1,2), xlab="R*", ylab="Brier score", las=1)
@
...
\begin{center}
%
<<label=rank-hist, fig.width=5, fig.height=2.5>>=
...
Ensemble dressing: \code{DressEnsemble}, \code{FitAkdParameters}, \code{EnsCrps}, \code{GetDensity}, \code{DressIgn}, \code{PlotDressedEns}

Auxiliary: \code{Detrend}, \code{GenerateToyData}

<<label=plot-dressed-ens>>=
with(GenerateToyData(N=20, mu.y=0, s.s=sqrt(.5), s.eps=sqrt(.5), 
                     mu.x=0, beta=sqrt(0.5), s.eta=sqrt(0.75), K=10, 
                     mu.x.ref=0, beta.ref=0, s.eta.ref=1, K.ref=20), 
     ScoreDiff(EnsCrps(ens, obs), EnsCrps(ens.ref, obs)))
@
